...
public class AddPatientRequestHandlerTests
{
    ...
    public async Task AddPatientAsync_ShouldReturnOk_WhenRequestIsSuccessful()
    {
        ...
        _mapperMock.Setup(m => m.Map<Patient>(It.IsAny<AddPatientRequest>()))
            .Returns(patient);

        _repositoriesMock.Setup(r => r.PatientRepository.AddAsync(It.IsAny<Patient>(), It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);

        _repositoriesMock.Setup(r => r.SaveAsync(It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);

        _mapperMock.Setup(m => m.Map<AddPatientResponse>(It.IsAny<Patient>()))
            .Returns(response);

        _mediatorMock.Setup(m => m.Send(It.IsAny<AddPatientRequest>(), It.IsAny<CancellationToken>()))
    ...
    public async Task AddPatientAsync_ShouldReturnInternalServerError_WhenUnexpectedExceptionOccurs()
    {
        ...
        // Act
        var result = await exceptionHandling.ExecuteWithExceptionHandling(() =>
            _controller.AddPatientAsync(request, CancellationToken.None));
